...
class EveLoader(object):
        ...
        for line in lines:
            ...
            c.execute(""" UPDATE alert
                          SET sid= %s, tmst='%s', type='%s', src_ip='%s', dest_ip='%s', src_port='%s', dest_port='%s', 
                              prot='%s', in_iface='%s', message='%s', category='%s', severity='%s', payload='%s', packet='%s' 
                          WHERE id='%s';
                      """ % ( sid, timestamp_formatted, ttype, src_ip, dest_ip, src_port, dest_port,
                              proto, in_iface, message, category, severity, payload, packet, str(self.curr_db_index)))
    ...
    def get_n_lines(self, n, page=0, order="timestamp_d", filter=default_filter):
        ...
        for row in c.execute("SELECT * FROM alert WHERE %s ORDER BY %s LIMIT %s OFFSET %s" % (f, o, n, n*page)):
            rows.append(map( str , row[:len(row)-2]))
        return rows

    def get_packet_by_id(self, idd):
        c = self.conn.cursor()
        rows = []
        for row in c.execute("SELECT payload, packet FROM alert WHERE id=%s" % idd):
            rows.append(row)
